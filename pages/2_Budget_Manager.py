...
if not existing.empty:
    limit_map = dict(zip(existing["category"], existing["weekly_limit"]))
...
if save:
    ...
    if errors:
        ...
        saved = sum(1 for cat, limit in new_limits.items() if limit > 0 and db.upsert_budget(cat, limit, week_id))
